...
// NewEtcd 获取redis实例
func NewEtcd(clientURI string, logger kitlog.Logger) Etcd {
	cli, err := v3.New(v3.Config{
		Endpoints:   strings.Split(clientURI, ","),
		DialTimeout: 5 * time.Second,
	})
	if err != nil {
		log.Fatalln("client connect error", clientURI)
	}
	return Etcd{KV: v3.NewKV(cli), log: kitlog.With(logger, "store", "etcd")}
}
...
func (p Etcd) Range(ctx context.Context, dataCenter uint8, db, table string, size int64) (int64, error) {
	...
	l := kitlog.With(p.log, "action", "range", "biz", biz, "size", size)
	...
	for i := 0; i < retryTimes; i++ {
		...
		case ErrEtcdNotFound:
			l.Log("action", "Get", "last", last, "err", err.Error())
			return 0, err
		default:
			l.Log("action", "Get", "last", last, "err", err.Error())
			continue
...
// last 获取上一次分配的数据
func (p Etcd) last(ctx context.Context, l kitlog.Logger, biz string) (int64, error) {
	// 获取旧的数据
...
// update 更新 etcd 存储数据
func (p Etcd) update(ctx context.Context, l kitlog.Logger, biz string, min int64, size int64) (int64, error) {
	var err error
	var resp *v3.TxnResponse

	now := strconv.FormatInt(min, 10)
	next := strconv.FormatInt(min+size, 10)
	l.Log("action", "Txn", "now", now, "next", next)
	// 新增还是更新
	if min == 0 {
	...
	if err != nil {
		l.Log("action", "Txn", "err", err.Error())
		return 0, err
	}
	if resp.Succeeded {
		return min, nil
